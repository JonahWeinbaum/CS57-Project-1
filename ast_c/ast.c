/*
 * AST Generator in C
 * Based on code from CS57 FA19
 *
 * NOTE: This contains C++ code to use the vector statement so compile with clang++-10 or g++
 *
 * 28 Apr 2022  jpb  Updates to code.
 */
...
char *get_indent_str(int n) {
    char *ret = (char *)calloc(4*(n + 1), sizeof(char));
    for (int i = 0; i < n; i++)
	strncat (ret, "|   ", 4);
    return ret;
}
...
void freeNode(astNode *node) {
    ...
    default: {
        fprintf(stderr, "freeNode: Incorrect node type %d\n", node->type);
        exit(1);
    }
    }
}
...
void freeStmt(astNode *node) {
    ...
    case ast_decl:
        ...
        break;
    default: {
        fprintf(stderr, "freeStmt: Incorrect node type %d\n", node->stmt.type);
        exit(1);
    };
    }
}
...
void printNode(astNode *node, int n) {
    ...
    default: {
        fprintf(stderr, "printNode: Incorrect node type\n");
        exit(1);
    }
    }
    free(indent);
...
void printStmt(astStmt *stmt, int n) {
    ...
    default: {
        fprintf(stderr, "printStmt: Incorrect node type\n");
        exit(1);
    }
    }
    free(indent);
